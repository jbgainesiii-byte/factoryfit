...
const visibleDataAfter = "2026-05-10T15:40:00.000Z";
...
function isVisibleTestData(row) {
  return !row.created_at || row.created_at >= visibleDataAfter;
}
...
export async function readCloudState() {
  ...
  return {
    ...
    submissions: submissionRows.filter(isVisibleTestData).map(normalizeSubmission),
    feedback: feedbackRows.filter(isVisibleTestData).map(normalizeFeedback),
